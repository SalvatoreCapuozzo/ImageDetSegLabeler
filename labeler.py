...
class ImageDetSegLabeler:
    def __init__(self, root):
        self.root = root
        self.root.title("Image Det & Seg Labeler")
    ...
    def setup_ui(self):
        ...
        tk.Button(control_frame, text="Load Classes", command=self.load_categories_file).pack(side=tk.LEFT, padx=5)
        ...
        # --- Bottom Status Bar ---
        self.coord_label = tk.Label(self.root, text="Ready. Right-click to probe coordinates.", bd=1, relief=tk.SUNKEN, anchor=tk.W, font=("Arial", 10, "bold"), bg="#222", fg="#0f0")
    ...
    def _parse_categories(self, filepath):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                # Handle formats like "ship = 1" or "1 = ship"
                if '=' in line:
                    parts = line.split('=')
                    try:
                        try:
                            c_id = int(parts[1].strip())
                            c_name = parts[0].strip()
                        except ValueError:
                            c_id = int(parts[0].strip())
                            c_name = parts[1].strip()
                        self.categories[c_id] = c_name
                    except ValueError:
    ...
    def load_folder(self):
        ...
        # Check for default categories.txt or classes.txt
        ...
        if self.mode_var.get() == "bbox":
            ...
            if len(self.current_polygon_points) >= 6:
                ...
                if dist * self.scale <= 15:
                    # FIX: Call the dedicated close method instead of simulating a right-click
                    self.close_mask() 
                    return
    ...
    def close_mask(self):
        """Helper to finalize and save the drawn polygon mask."""
    ...
    def on_right_click(self, event=None):
        ...
        self.coord_label.config(text=f"Probed: {pixel_str}", fg="#0f0")
        
        self.canvas.create_oval(px-4, py-4, px+4, py+4, fill="yellow", outline="black", width=2, tags="temp_coord")
        self.canvas.create_text(px+2, py-15, text=pixel_str, fill="black", tags="temp_coord", font=("Arial", 10, "bold"), justify=tk.CENTER)
        self.canvas.create_text(px, py-17, text=pixel_str, fill="yellow", tags="temp_coord", font=("Arial", 10, "bold"), justify=tk.CENTER)
    ...
    def refresh_sidebar_labels(self):
        ...
        cat_list = [f"{k} - {v}" for k, v in self.categories.items()] if self.categories else ["0 - Default"]
        ...
        for ann in self.annotations:
            ...
            combo = ttk.Combobox(row, textvariable=c_var, values=cat_list, state="readonly", width=14)
    ...
    def update_legend_and_dropdowns(self):
        ...
        if not self.categories:
            tk.Label(self.legend_frame, text="No classes loaded", fg="black", bg="#f0f0f0").pack(anchor=tk.W)
            cat_list = ["0 - Default"]
        else:
            cat_list = []
    ...
    def load_image_by_index(self):
        ...
        self.root.title(f"Generic Labeler | [{self.current_idx + 1}/{len(self.image_list)}] - {filename}")
    ...
    def load_existing_annotations(self):
        ...
        # 1. Check for JSON Annotations
        json_path = os.path.join(dir_name, base_name + ".json")
        ...
        # 2. Check for YOLO Annotations (txt files)
        yolo_path = os.path.join(dir_name, base_name + ".txt")
        if not os.path.exists(yolo_path):
            yolo_path = os.path.join(dir_name, "..", "labels", base_name + ".txt")
            ...
            try:
                ...
                for line in lines:
                    ...
                    if len(parts) == 5:
                        ...
                        x_c, y_c, w_n, h_n = parts[1:5]
                        w = w_n * self.img_width
                        h = h_n * self.img_height
                        xmin = (x_c * self.img_width) - (w / 2.0)
                        ymin = (y_c * self.img_height) - (h / 2.0)
                        ...
                        # YOLO Mask (class x1 y1 x2 y2 ...)
                        pts = parts[1:]
                        scaled_pts = []
                        for i in range(0, len(pts), 2):
                            scaled_pts.append(pts[i] * self.img_width)
                            scaled_pts.append(pts[i+1] * self.img_height)
                            
                        self.annotations.append({
                            "id": self.current_id,
                            "category_id": cat_id,
                            "segmentation": [scaled_pts],
                            "type": "segmentation"
                        })
                        self.current_id += 1
            except Exception as e:
                print(f"Error loading YOLO txt: {e}")
